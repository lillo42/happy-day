...
func TestCreateEndpoint_Should_ReturnInvalidBody_When_BodyIsNotJson(t *testing.T) {
	...
	req, _ := http.NewRequest(http.MethodPost, "/api/customers", bytes.NewBuffer([]byte(`"jsonStr"`)))
...
func TestCreateEndpoint_Should_ReturnError_When_ErrorToExecuteCreate(t *testing.T) {
	...
	b, _ := json.Marshal(CreateRequest{Name: name, Comment: comment, Phones: phones})
	req, _ := http.NewRequest(http.MethodPost, "/api/customers", bytes.NewBuffer(b))
